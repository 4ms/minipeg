...
void update_pwm(uint32_t pwmval, enum PwmOutputs pwmnum) {
	...
	if (channel==TIM_CHANNEL_1)
		p[pwmnum].tim.Instance->CCR1 = pwmval;

	else if (channel==TIM_CHANNEL_2)
		p[pwmnum].tim.Instance->CCR2 = pwmval;

	else if (channel==TIM_CHANNEL_3)
		p[pwmnum].tim.Instance->CCR3 = pwmval;

	else if (channel==TIM_CHANNEL_4)
		p[pwmnum].tim.Instance->CCR4 = pwmval;

	else if (channel==TIM_CHANNEL_5)
		p[pwmnum].tim.Instance->CCR5 = pwmval;

	else if (channel==TIM_CHANNEL_6)
		p[pwmnum].tim.Instance->CCR6 = pwmval;
}
...
void populate_pwm_pins(struct PWMOutput p[]) {
	...
	p[PWM_ENVLED_G].af = GPIO_AF5_TIM3;
	...
	p[PWM_PINGBUT_B].af = GPIO_AF2_TIM6;
