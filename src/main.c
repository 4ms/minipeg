...
int main(void) {
  ...
  if (settings.start_clk_time) {
    ...
    a.div_clk_time =
        settings.start_clk_time; // Todo: settings.start_clk_time_aux;
  }
...
static void read_ping_button(void) {
  if (digin[PING_BUTTON].state == 1) {
    ...
    if (digin[PING_BUTTON].edge == 1) {
      ...
      if (last_tapin_time &&
          (diff32(last_tapin_time, now) < (last_tapin_time >> 1))) {
...
void handle_qnt_trig(struct PingableEnvelope *e) {
  ...
  if (QNT_REPHASES_WHEN_CYCLE_OFF || cycle_but_on || !e->envelope_running) {
    e->ping_div_ctr =
        e->clock_divider_amount; // make sure divided clock envelopes start from
...
void check_reset_envelopes(void) {
  ...
  if (m.div_clk_time) {
    if ((!div_ping_led) &&
        (m.divpingtmr > m.div_clk_time)) { // Todo: why do we have !div_ping_led
                                           // && ( > ). Do we need that?
    ...
    */
    if (clk_time && !settings.free_running_ping && !using_tap_clock) {
      ...
      if (pingtmr >= ext_ping_timeout) {
        ...
        if (tapout_clk_time &&
            (settings.trigout_function == TRIGOUT_IS_TAPCLKOUT)) {
          // switch to tap clock
          using_tap_clock = 1;
